...
export function AdminPhantomRequestsSection({
  ...
  function complete(requestId: string) {
    start(async () => {
      ...
      if (res.ok) {
        toast.success("Mode Fantôme terminé");
      } else {
        toast.error(res.error);
      }
    });
    ...
    <section className="space-y-3">
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-semibold tracking-tight">Mode Fantôme</h2>
        </div>
      ...
      {requests.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-border/60 px-4 py-8 text-center text-sm text-muted-foreground">
          Aucune demande Mode Fantôme à traiter.
        </p>
      ) : (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {request.admin_note ? (
                      <p className="mt-2 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-zinc-300">
                    ...
                    <button
                      ...
                      onClick={() =>
                        updateStatus(
                          request.id,
                          "payment_pending",
                          "Paiement Mode Fantôme en attente",
                        )
                      }
                      className={cn(
                        buttonVariants({ variant: "default", size: "lg" }),
                      ...
                      <button
                        ...
                        onClick={() =>
                          updateStatus(
                            request.id,
                            "in_progress",
                            "Intervention Mode Fantôme en cours",
                          )
                        }
                        className={cn(
                          buttonVariants({ variant: "default", size: "lg" }),
